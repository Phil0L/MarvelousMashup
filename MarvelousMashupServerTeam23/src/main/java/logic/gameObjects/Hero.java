...
public class Hero extends Placeable implements Attackable {
  ...
  public ConfigHero toConfigHero(Configuration config) {
    return new ConfigHero(
        Arrays.stream(config.characterConfig.characters).anyMatch(c -> c.name.equals(this.name))
            ? Arrays.stream(config.characterConfig.characters).filter(c -> c.name.equals(this.name)).findFirst().get().characterID
            : this.ID,
        this.name, this.maxHealthPoints, this.maxMovementPoints,
