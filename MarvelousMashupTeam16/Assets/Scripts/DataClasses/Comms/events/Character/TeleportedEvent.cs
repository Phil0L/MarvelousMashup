public class TeleportedEvent : Message, CharacterEvent
{
    /**
    ...
    public TeleportedEvent(IDs teleportedEntity, int[] originField, int[] targetField,
        IDs originPortal, IDs targetPortal) : base(EventType.TeleportedEvent)
    {
    ...
    public void Execute()
    {
        var character = IDTracker.Get(teleportedEntity) as Character;
        if (character != null)
        {
            Game.State().TeleportHero(character.characterID, targetField.ToVector());
        }
    }
}