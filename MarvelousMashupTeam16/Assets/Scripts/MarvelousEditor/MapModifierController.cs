...
namespace MarvelousEditor
{
    public class MapModifierController : MonoBehaviour
    {
        ...
        private List<List<DataClasses.MapTile>> _changableMap = new List<List<DataClasses.MapTile>>();
        ...
        void StoreUpdate(DataClasses.Map map, MapStore.MapAction action)
        {
        ...
        public void TileClicked(int x, int y)
        {
            ...
            switch (toolsManager.selectedTool)
            {
                case MapToolsManager.Tool.Switch:
                    if (_changableMap[x][y] == DataClasses.MapTile.ROCK)
                        _changableMap[x][y] = DataClasses.MapTile.GRASS;
                    else
                        _changableMap[x][y] = DataClasses.MapTile.ROCK;
                    break;
                ...
                case MapToolsManager.Tool.Portal:
                    _changableMap[x][y] = DataClasses.MapTile.PORTAL;
                    break;
    ...
    {
        public static void NewSize(this List<List<DataClasses.MapTile>> map, int newWidth, int newHeight)
        {
            foreach (List<DataClasses.MapTile> row in map)
            {
                row.Resize(newHeight, DataClasses.MapTile.GRASS);
            }   
            map.ResizeWithList(newWidth, Enumerable.Repeat(DataClasses.MapTile.GRASS, newHeight).ToList());
        }
        ...
        private static void ResizeWithList(this List<List<DataClasses.MapTile>> list, int sz, List<DataClasses.MapTile> c)
        {
            int cur = list.Count;
            if(sz < cur)
                list.RemoveRange(sz, cur - sz);
            else if(sz > cur)
            {
                ...
                {
                    List<DataClasses.MapTile> cc = c.Clone() as List<DataClasses.MapTile>;
                    list.Add(cc);
                }
            }
        }
    ...
    {
        public static DataClasses.Map ToMap(this List<List<DataClasses.MapTile>> map)
        {
            return new DataClasses.Map(map.Select(Enumerable.ToArray).ToArray());
        }
    }
}